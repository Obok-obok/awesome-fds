...
본 시스템은 “모델 성능”이 아니라 **“심사 정책 효과”를 검증·확장하는 운영 체계**로 설계
...
의사결정은 **재무 효과 → 통계적 신뢰 → 운영 리스크(Guardrail)** 순으로 진행
- **재무 효과(절감)**: 처리군 vs 통제군 지급액 차이로 절감 규모  
- **유의성(신뢰)**: Welch t-test로 우연 가능성(p-value) 통제  
- **확장 전략**: HTE로 세그먼트별 효과 차이를 확인해 선택적 확대 가능  
- **리스크 통제**: Guardrail 미충족 시 GO 금지(hold/rollback)하도록 설계
